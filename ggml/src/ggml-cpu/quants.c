...
void quantize_row_tkv2_0(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    assert(k % QK_TKV_0 == 0);
    block_tkv2_0 * GGML_RESTRICT y = vy;
    quantize_row_tkv2_0_ref(x, y, k);
}

void quantize_row_tkv3_0(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    assert(k % QK_TKV_0 == 0);
    block_tkv3_0 * GGML_RESTRICT y = vy;
    quantize_row_tkv3_0_ref(x, y, k);
}

void quantize_row_tkv4_0(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    assert(k % QK_TKV_0 == 0);
    block_tkv4_0 * GGML_RESTRICT y = vy;
    quantize_row_tkv4_0_ref(x, y, k);
}
...
void ggml_vec_dot_pq2_0_q8_0_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int k = 0; k < 4; ++k) {
            ...
            for (int b = 0; b < 8; ++b) {
                const uint8_t q = qs[b];
                sumi_block += ((int) ((q >> 0) & 3) - 1) * qy[b*4 + 0];
                sumi_block += ((int) ((q >> 2) & 3) - 1) * qy[b*4 + 1];
                sumi_block += ((int) ((q >> 4) & 3) - 1) * qy[b*4 + 2];
                sumi_block += ((int) ((q >> 6) & 3) - 1) * qy[b*4 + 3];
            }

            sumi += GGML_CPU_FP16_TO_FP32(yb->d) * sumi_block;
        }
