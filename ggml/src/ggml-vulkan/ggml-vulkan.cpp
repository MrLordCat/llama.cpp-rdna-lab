...
static void ggml_vk_load_shaders(vk_device& device) {
    ...
    if (device->coopmat2) {
        ...
        if ((device->architecture == AMD_GCN) && (device->driver_id != vk::DriverId::eAmdProprietary)) {
        ...
        } else if (device->vendor_id == VK_VENDOR_ID_AMD && device->coopmat_support && (device->driver_id != vk::DriverId::eAmdProprietary || force_amd_large_matmul)) {
            ...
            if (amd_large_matmul_variant != nullptr) {
                ...
                const std::string variant(amd_large_matmul_variant);
                if (variant == "wmiter1") {
                    l_warptile_mmq[6] = 1;
                    l_warptile_mmq_int[6] = 1;
                } else if (variant == "block128") {
                    l_warptile_mmq[0] = 128;
                    l_warptile_mmq_int[0] = 128;
                } else if (variant == "block512") {
                    l_warptile_mmq[0] = 512;
                    l_warptile_mmq_int[0] = 512;
                } else if (variant == "block64") {
                    l_warptile_mmq[0] = 64;
                    l_warptile_mmq_int[0] = 64;
                } else if (variant == "wm128") {
                    l_warptile_mmq[4] = 128;
                    l_warptile_mmq_int[4] = 128;
                } else if (variant == "wn32") {
                    l_warptile_mmq[5] = 32;
                    l_warptile_mmq_int[5] = 32;
                } else if (variant == "wn16") {
                    l_warptile_mmq[5] = 16;
                    l_warptile_mmq_int[5] = 16;
                } else if (variant == "wn48") {
                    l_warptile_mmq[5] = 48;
                    l_warptile_mmq_int[5] = 48;
                } else if (variant == "wn96") {
                    l_warptile_mmq[5] = 96;
                    l_warptile_mmq_int[5] = 96;
                } else if (variant == "block128-wn32") {
                    l_warptile_mmq[0] = 128;
                    l_warptile_mmq[5] = 32;
                    l_warptile_mmq_int[0] = 128;
                    l_warptile_mmq_int[5] = 32;
                } else if (variant == "wn32-wmiter1") {
                    l_warptile_mmq[5] = 32;
                    l_warptile_mmq[6] = 1;
                    l_warptile_mmq_int[5] = 32;
                    l_warptile_mmq_int[6] = 1;
                } else if (variant == "block128-wn32-wmiter1") {
                    l_warptile_mmq[0] = 128;
                    l_warptile_mmq[5] = 32;
                    l_warptile_mmq[6] = 1;
                    l_warptile_mmq_int[0] = 128;
                    ...
                    l_warptile_mmq_int[1] = 64;
                    l_warptile_mmq_int[2] = 64;
                } else if (variant == "block128-bn64") {
                    l_warptile_mmq[0] = 128;
                    l_warptile_mmq[2] = 64;
                    l_warptile_mmq_int[0] = 128;
                    l_warptile_mmq_int[2] = 64;
                } else if (variant == "block128-wm128") {
                    l_warptile_mmq[0] = 128;
                    l_warptile_mmq[4] = 128;
                    l_warptile_mmq_int[0] = 128;
                    l_warptile_mmq_int[4] = 128;
        ...
        if (amd_large_matmul_variant_active) {
            const std::string variant(amd_large_matmul_variant);
            if (variant == "bm64") {
...
static vk_device ggml_vk_get_device(size_t idx) {
    ...
    if (vk_instance.devices[idx] == nullptr) {
        ...
        const char * max_nodes_per_submit = getenv("GGML_VK_MAX_NODES_PER_SUBMIT");
        if (max_nodes_per_submit != nullptr) {
            device->max_nodes_per_submit = std::max((uint32_t) std::stoul(max_nodes_per_submit), 1u);
        }
        ...
        for (uint32_t i = 0; i < GGML_TYPE_COUNT; ++i) {
            ...
            device->mul_mat_l[i]    = device->coopmat_support && (device->driver_id != vk::DriverId::eAmdProprietary || force_amd_large_matmul);
...
static void ggml_vk_instance_init() {
    ...
    if (std::getenv("GGML_VK_DEVICE_GROUP_TRACE") != nullptr) {
        uint32_t group_count = 0;
        VK_CHECK((vk::Result) vkEnumeratePhysicalDeviceGroups((VkInstance) vk_instance.instance, &group_count, nullptr),
            "vkEnumeratePhysicalDeviceGroups(count)");
        std::vector<VkPhysicalDeviceGroupProperties> groups(group_count);
        for (VkPhysicalDeviceGroupProperties & group : groups) {
            group.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_GROUP_PROPERTIES;
            group.pNext = nullptr;
        }
        VK_CHECK((vk::Result) vkEnumeratePhysicalDeviceGroups((VkInstance) vk_instance.instance, &group_count, groups.data()),
            "vkEnumeratePhysicalDeviceGroups(data)");
        for (uint32_t i = 0; i < group_count; ++i) {
            ...
            GGML_LOG_INFO("ggml_vulkan: device group %u: devices=%u subset_allocation=%u names=[%s]\n",
                i, groups[i].physicalDeviceCount, groups[i].subsetAllocation, names.str().c_str());
        }
    }
