...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_pq2_0(
    ...
    constexpr int blocks_per_iter = MMQ_ITER_K / QKPQ2_0;
    constexpr int threads_per_row = blocks_per_iter * QIPQ2_0;
    constexpr int nrows = warp_size / threads_per_row;
    constexpr int scale_entries_per_block = QKPQ2_0 / QK8_1;
    constexpr int scale_entries_per_row = blocks_per_iter * scale_entries_per_block;

    const int txi = threadIdx.x % threads_per_row;
    const int kbx = txi / QIPQ2_0;
    const int kqsx = txi % QIPQ2_0;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const int offset = 8*kqsx;
        const int qs0 = bxi->qs[offset + 0] | (bxi->qs[offset + 1] << 8) |
                        (bxi->qs[offset + 2] << 16) | (bxi->qs[offset + 3] << 24);
        const int qs1 = bxi->qs[offset + 4] | (bxi->qs[offset + 5] << 8) |
                        (bxi->qs[offset + 6] << 16) | (bxi->qs[offset + 7] << 24);

        const int dst_offset = kbx*(scale_entries_per_block*QI8_0) + kqsx*QI8_0;
#pragma unroll
        for (int j = 0; j < 4; ++j) {
#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
            x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + dst_offset + j] = pq2_expand_s2x4(qs0 >> (8*j));
#else
            x_qs[i*(2*MMQ_TILE_NE_K + 1) + dst_offset + j] = pq2_expand_s2x4(qs0 >> (8*j));
#endif
        }
#pragma unroll
        for (int j = 0; j < 4; ++j) {
#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
            x_qs[i*MMQ_MMA_TILE_X_K_Q8_0 + dst_offset + 4 + j] = pq2_expand_s2x4(qs1 >> (8*j));
#else
            x_qs[i*(2*MMQ_TILE_NE_K + 1) + dst_offset + 4 + j] = pq2_expand_s2x4(qs1 >> (8*j));
#endif
        }
    }

    if (threadIdx.x < scale_entries_per_row) {
        const int ksx = threadIdx.x;
        const int scale_block = ksx / scale_entries_per_block;
...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_q3_K(
    const char * __restrict__ x, int * __restrict__ x_tile, const int kbx0, const int i_max, const int stride) {
    load_tiles_q3_K_impl<block_q3_K, mmq_y, need_check>(x, x_tile, kbx0, i_max, stride);
}
...
static __device__ __forceinline__ void mul_mat_q_load_tiles(
    ...
    if constexpr (type == GGML_TYPE_Q3_K) {
        ...
        GGML_UNUSED(use_q3k_padded_storage);
    }
...
static __device__ __forceinline__ void mul_mat_q_process_tile(
    ...
    const bool use_rdna4_moe_mmq_staging, const bool use_q3k_padded_storage) {
    ...
    static_assert(!use_dp4a || type == GGML_TYPE_Q3_K || type == GGML_TYPE_Q8_0,
        "The small-N DP4A route is Q3_K-only (Q8_0 enabled for D094 c8 experiments).");
...
static __global__ void mul_mat_q(
    ...
    if constexpr (use_dp4a) {
        static_assert((type == GGML_TYPE_Q3_K || type == GGML_TYPE_Q8_0),
            "Only the Q3_K/Q8_0 small-N DP4A specializations are supported (D094).");
        if constexpr (type != GGML_TYPE_Q3_K && type != GGML_TYPE_Q8_0) {
            NO_DEVICE_CODE;
        }
    } else {
...
struct mmq_args {
    ...
    bool use_stream_k; int64_t ncols_max; bool q3k_padded_storage;
};
...
static void launch_mul_mat_q(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream, const bool use_rdna4_moe_mmq_staging) {
    ...
    // D094 c8 experiment: force the DP4A variant for Q8_0 (mirrors ROCm mma
    // numerics comparison; dp4a layout/vec_dot exist for Q8_0).
    const bool force_dp4a = type == GGML_TYPE_Q8_0 && std::getenv("GGML_CUDA_FORCE_DP4A") != nullptr;

    const int nbytes_shared = mmq_get_nbytes_shared<type>(mmq_x, mmq_y, cc, warp_size, nwarps, use_rdna4_moe_mmq_staging, force_dp4a);
                ...
                if (force_dp4a) {
                    mul_mat_q<type, mmq_x, need_check, true><<<block_nums_xy_tiling, block_dims, nbytes_shared, stream>>>
                        (args.x, args.y, args.ids_dst, args.expert_bounds, args.dst, nullptr,
                         ...
                         channel_ratio_fd, nchannels_y_fd, args.stride_channel_x, args.stride_channel_y, args.stride_channel_dst,
                         sample_ratio_fd, nsamples_y_fd, args.stride_sample_x, args.stride_sample_y, args.stride_sample_dst,
                         ntx_fd, use_rdna4_moe_mmq_staging, args.q3k_padded_storage);
                } else {
...
void mul_mat_q_case(ggml_backend_cuda_context & ctx, const mmq_args & args, cudaStream_t stream) {
    ...
    const bool use_rdna4_moe_mmq_staging_effective = false;
...
#undef GGML_MMQ_LAUNCH_CASE
    ...
    if (trace_timing) {
        ...
        GGML_LOG_INFO(
            "%s: timing type=%d cc=%d nrows_x=%lld ncols_max=%lld ncols_dst=%lld mmq_x_best=%d mmq_x_forced=%d mmq_y=%d q3k_padded=%d smalln_dp4a=%d sync_req=%d pre_sync_applied=%d sync_applied=%d capture=%d trace_resources=%d block_threads=%d nbytes_shared=%d smpbo=%zu shared_pct=%.2f regs=%d static_shared=%zu max_dyn_shared=%d max_blocks_per_sm=%d max_threads_per_sm=%d occupancy_pct=%.2f waves_per_sm=%.2f pre_sync_ms=%.3f enqueue_ms=%.3f sync_ms=%.3f total_ms=%.3f\n",
            __func__,
            (int) type,
            cc,
    ...
    if (trace_path) {
        GGML_LOG_INFO(
            "%s: type=%d cc=%d ncols_max=%lld mmq_x_max=%d mmq_y=%d nwarps=%d mmq_x_best=%d mmq_x_forced=%d ntiles_x_best=%d q3k_padded=%d smalln_dp4a=%d trace_resources=%d block_threads=%d nbytes_shared=%d smpbo=%zu shared_pct=%.2f regs=%d max_blocks_per_sm=%d occupancy_pct=%.2f waves_per_sm=%.2f\n",
            __func__,
            (int) type,
            cc,
            (long long) args.ncols_max,
