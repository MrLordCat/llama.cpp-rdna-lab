...
17. H21 was neutral in E048: `ROCBLAS_USE_HIPBLASLT=1` was only `+0.10%` aggregate and `+0.04%` prompt on `ubatch=2048`, so treat it as noise and do not promote.
...
22. H26 was rejected in E055 and the prototype code was reverted. Trace-off r1 was `11.86 TPS`, but split-detail showed only `-1.58%` Q3_K `src0_convert_ms` and `-0.41%` on the target `6144x5120@ncols2048` conversion, far below the E053 `>=25%` local keep gate.
