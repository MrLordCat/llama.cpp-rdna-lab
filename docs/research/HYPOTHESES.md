...
11. H34 is kept as a 12k Q3 decode-heavy profile, not a ROCm prompt-heavy default. Post-driver E116 measured Vulkan f16 decode r3 at `40.2753 TPS` aggregate / `41.2283 tok/s` decode, and E119 confirmed the route on real-context 512-token session output: `32.0298 TPS` aggregate / `33.89 TPS` warm-only vs ROCm q4 `24.9524` / `25.82`. E118 live-server sanity found normal thinking/answers with no `wm32-wn32`-style corruption. E117 rejects f16/q8 KV for the cold prompt-heavy ROCm default (`q4 11.9858`, f16 `11.9028`, q8 `11.6392`).
...
- Kept profile change: H34 / E116+E118+E119, Q3_K_S ctx=12288 Vulkan f16 decode-heavy route; r3 decode gate `40.2753 TPS` aggregate / `41.2283 tok/s` decode, and real-context 512-token session r3 `32.0298 TPS` aggregate / `33.89 TPS` warm-only. Live-server output sanity was accepted by the user. E117 keeps ROCm prompt-heavy default on q4 KV.
