...
| H28 | RDNA4 selector parity audit vs upstream #18816 | External RDNA4 selector work changed MMQ/MMVQ thresholds by qtype, and local thresholds now differ; this may expose small stackable wins on activated non-Q3-large shapes without repeating broad force-MMQ | 0% to +2% current lane if affected shapes have share; larger on other quants/lanes | duplicate of E050 if applied to Q3_K `ne11=2048`; extra VRAM or route regressions if promoted too broadly | trace local shapes, diff against #18816 routing table, then run only narrow type/shape gates |
| H29 | gfx12 WMMA direct quantized prefill design | hipfire/R9700 work reports large gfx1201 WMMA prefill gains, but the useful lesson is changed K packing/thread mapping rather than current llama.cpp MMQ routing | 0% to +5% current wall if a new Q3_K direct kernel beats dequant+GEMM; higher on compatible HFQ4/AWQ-style formats | high complexity, likely duplicate of rejected E050 unless the kernel design materially changes packing/K mapping | design gate against E049 cuBLAS `1839.27 ms` and E050 MMQ `2529.35 ms` before any code |
...
24. H28 was opened by E059 after external RDNA4 research. It is a cheap selector-diff audit only; do not rerun broad `GGML_CUDA_FORCE_MMQ_RUNTIME=1`, and do not apply it to Q3_K `ne11=2048` because E050 already rejected that exact route.
25. H29 was opened by E059 as a high-risk design track. It should not get code until a design gate shows how a gfx12 direct quantized prefill kernel differs materially from current MMQ and can plausibly beat the E049/E050 target timings.
