...
| Latest completed research program | [D096 native FP8 and platform-tail roadmap](../D096_ROADMAP.md), complete: D098/D099 production ROCm FP8 plus independent explicit MTP draft FP8; Vulkan D5/R9 retained only as reopen-only diagnostics |
| Latest execution-gap program | [D100 ROCm long-context execution gap](D100_ROCM_LONG_CONTEXT_EXECUTION_GAP.md), complete: stable HIP replay and about `0.6 ms` host launch reject a graph overhaul; N=1 device graphs own `18.829/19.642 ms` at 49K and `21.329/22.192 ms` at 98K; bounded full-FP8 launch-topology probes were neutral or negative, so observability only was retained |
| Latest kernel-ceiling program | [D102 ROCm full-FP8 fused-body phase census](D102_ROCM_FP8_FUSED_BODY_PHASE_CENSUS.md), complete: decode kernel splits `55.4%` P*V / `28.3%` KQ / `14.3%` softmax+requant / `2.0%` merge on both devices; no phase owns 60%, so no prototype was admitted |
| Active research program | [R001 RDNA4 architecture exploitation for the ROCm backend](../rdna4-architecture/README.md), phase 1 (research only, no builds/benches): audit the CUDA-centric assumptions in `ggml-cuda` against the actual gfx1201 ISA; first facts show the full-native decode kernel at 248 VGPR = one CTA per CU (VGPR-bound occupancy) |
| Deferred | [D103 ROCm P*V phase instruction/register cost](D103_ROCM_VKQ_PART_LDS_TRAFFIC.md), deprioritized: superseded by R001 |
...
| Status | Parked secondary model-specific program while D096 is the active primary queue; reopen only explicitly and repeat the adjacent baseline first |
...
| Status | Closed and parked on 2026-08-13. D002/D013-D027 close the ROCm body fence; D028-D033 close the later Vulkan 2.4 topology gate. |
...
## P002 Candidate Queue (closed historical queue)
