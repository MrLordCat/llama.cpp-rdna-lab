...
| T10 | Q4/Q3 tool-call reliability guard | D038 promotes the guard to default-on after improving the D038 q4 tool-call workload from `2/4` to `4/4`; disable with `LLAMA_SERVER_TOOL_CALL_THINKING_GUARD=0` for A/B | broaden tool-call tests after default promotion; next implementation should try fallback/retry only after no-tool-call/length failures so normal thinking remains available where explicitly requested |
