...
| Latest kernel-ceiling program | [D102 ROCm full-FP8 fused-body phase census](D102_ROCM_FP8_FUSED_BODY_PHASE_CENSUS.md), complete: decode kernel splits `55.4%` P*V / `28.3%` KQ / `14.3%` softmax+requant / `2.0%` merge on both devices; no phase owns 60%, so no prototype was admitted and the fp32 VKQ-part LDS traffic in the P*V phase is the next candidate boundary |
| Active research program | [D103 ROCm P*V phase instruction/register cost](D103_ROCM_VKQ_PART_LDS_TRAFFIC.md), planning phase: registers, not LDS, bound the native P*V phase (D098 bisect); a 10% phase cut is worth ~1.1% decode at 49K but ~2.2% at 98K, so candidates are validated 98K-first |
