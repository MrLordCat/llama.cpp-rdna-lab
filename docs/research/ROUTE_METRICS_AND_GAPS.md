...
| --- | --- | --- | --- |
| ROCm large Q3_K prefill through `cublas_backend` / hipBLAS | E045/E053/E056/E058 baseline cluster around `11.65-11.77 TPS`; E049 split: `src0 32.29%`, `src1 6.74%`, GEMM `60.97%`; E054: `src0_convert_ms=3370.32`, target convert `1430.88`; E103: `2792` Q3_K route rows, `349` keys, each repeated `8x`. E191 post-pairdot real-context trace shows parsed `MUL_MAT forward` `4944.803 ms`, dominated by `cublas_backend|q3_K` `3891.530 ms` (`78.70%`), with MMVQ only `148.009 ms` on the short diagnostic run. E192 split refresh shows Q3_K split `5213.358 ms`: `src0_convert_ms=1637.070`, `src1_ms=364.309`, `gemm_ms=3203.883`; top shapes are `(17408x5120)@2048` `1425.411 ms`, `(5120x17408)@2048` `901.308 ms`, `(6144x5120)@2048` `879.019 ms`, and `(10240x5120)@2048` `751.862 ms`. Q3_K route rows: `1396`, unique keys: `698`, all repeated, max `4` calls/key | Current route repeatedly stages Q3_K weights to fp16 before rocBLAS. GEMM is useful and remains most of the Q3_K split, while `(6144x5120)` / `attn_gate` is conversion dominated. Persistent fp16 cache would save conversion but costs residency and already regressed; existing MMQ forcing is not competitive for these large-N shapes | Main practical ROCm wall target after E191/E192. Do not force existing MMQ, compute16, hipBLASLt, or persistent fp16 cache. Next viable work is a non-persistent fused/direct Q3_K x F16 route, or graph scheduling that avoids repeated source staging without broad fp16 residency |
...
| ROCm decode parity vs Vulkan | E116 post-driver decode-focused gate: ROCm q4 `29.1685 TPS` / `29.625 tok/s`; Vulkan q4 r3 `39.8801 TPS` / `40.8683 tok/s`; Vulkan f16 r3 `40.2753 TPS` / `41.2283 tok/s`. E149 audit computes a first target of about `1.38x` decode-eval speedup. Old C01 trace says `MUL_MAT forward+fused` was about `61.9%` of parsed CUDA-node time, while `RMS_NORM+ROPE+SET_ROWS` was only `8.4%`. Fresh E149 ROCm non-sync trace has `MUL_MAT forward` `77.84%`, `FLASH_ATTN_EXT` `5.29%`, and `RMS_NORM+ROPE+SET_ROWS` only `3.26%`; route counts include `mul_mat_vec_q_direct,q3_K` `929` and `mul_mat_q_direct,q3_K` `349`. Parsed post-initial ROCm sections are still matmul-led (`MUL_MAT forward+fused 68.57%`) but show FA as secondary (`17.43%`). E149 sync companion (`max_tokens=16`, graph disabled, first section excluded) moderates the route share to `MUL_MAT forward+fused 53.80%`, `RMS_NORM+ROPE+SET_ROWS 11.83%`, and `FLASH_ATTN_EXT 1.46%`. Decode-only Q3_K route-delta tooling then narrows the target: ROCm parsed Q3_K is `mul_mat_vec_q_fused 63.78%` and `mul_mat_vec_q_direct 36.22%`; top normalized shapes are `m=17408,n=1,k=5120` (`36.07%` ROCm, `48.32%` Vulkan), `m=5120,n=1,k=17408` (`24.33%` / `25.47%`), `m=10240,n=1,k=5120` (`13.88%` / `10.99%`), and `m=6144,n=1,k=5120` (`11.23%` / `7.32%`). E150 rejects disabling fusion: clean `30.08 tok/s` decode vs `28.61 tok/s` with `GGML_CUDA_DISABLE_FUSION=1`. E151 keeps RDNA4 Q3_K `nwarps=2`: clean post-rebuild r3 `28.1123 TPS` / `29.77 tok/s` vs candidate r3 `30.3145 TPS` / `32.2467 tok/s`; live `llama-server` sanity produced normal thinking text. E152 fixes route-delta shape normalization for MMVQ `rows_per_block=2` and confirms the post-E151 residual split is still Q3_K fused `64.62%` / direct `35.38%`; top corrected shapes remain `m=17408,n=1,k=5120` and `m=5120,n=1,k=17408`. Vulkan q4 perf comparator decode sections are also Q3-led: Q3_K `MUL_MAT_VEC 50.67%` plus Q3_K `MUL_MAT_ADD_VEC 19.38%`; `RMS_NORM_MUL 3.79%`, `FLASH_ATTN_EXT 1.11%`, `ROPE+SET_ROWS 0.60%`. Sequential graph-disable control tied/noise (`27.1129` vs `27.2063 TPS`) | The gap is real, but the proposed root cause "ROCm has no fusion/pipeline cache" is not supported by code. ROCm already has RMS, rope/set-rows, unary, SSM, FFN decode fusions and HIP graph capture. Existing fusion helps, and E151 proves MMVQ launch policy can move the lane. E152 shows the next route must be larger than a nearby selector toggle: sync topology still concentrates on fused/direct Q3_K. Norm/rope cleanup is secondary and 64k decode still needs a separate FA/KV trace | Keep E151. H39 remains open: remaining gap to Vulkan q4 is about `1.27x`. Next branch should inspect the fused `mul_mat_vec_q` Q3_K FFN path, resource pressure, and row-pair scheduling before coding a larger RDNA4/Qwen-specific route. Do not start with standalone norm/rope fusion unless a future trace gives it a real ceiling |
...
| FlashAttention on Vulkan 64k | E128 perf trace: `FLASH_ATTN_EXT 33965.16 ms`, `38.03%` of traced Vulkan time. E131 route trace: `flash_attn_f32_f16_aligned_f32accq4_0`, `coopmat1`, q4/q4, `Br=16,Bc=64,D_split=8,row_split=4`, main `N=1024`, `KV=1024..57344`, `split_k=1`, `use_mask_opt=1`. E132 resource stats: main route `98 VGPR`, `76 SGPR`, `26112 B LDS`, `0 scratch`; forced SHMEM staging fell back to scalar and prompt dropped to `520.18`. E133 shape summary shows largest tail chunks around `KV=55k-57k` are `1.12-1.17 s` each across 16 calls. E134 route ceiling says FA alone would need `1.494x` local speedup to match the ROCm 64k wall alone. E138 forced existing split-k from `KV>=8192`; route trace showed `split_k=2`, but prompt fell `666.87 -> 96.29 tok/s`. E145 retuned `D_split=4/16` inside the same cm1 route and regressed pp7488 `978.88 -> 953.24/951.54` with identical reported resources. E148 analytic-causal-mask gate found `99.71%` of FA time eligible and a `51072.00 MiB` mask-prepass read proxy, but the env prototype only moved pp7488 `971.41 -> 972.21` | At 64k FA becomes a first-class bottleneck, but the easy toggles are already negative. Mask optimization pays for itself, split-k is not active on main chunks because the default already has enough row/head workgroups, f16acc does not move full wall TPS, SHMEM staging does not fit as a valid coopmat1 route on the current driver, existing split-k adds temp writes plus sync/reduce per FA node, simple `D_split` retunes do not turn lower live output state into lower measured resources, and removing the mask-opt prepass while preserving causal skip semantics is only noise-level. The remaining cost is likely the main q4 K/V dequant + softmax/PV shader loop over long KV, not a trivial tile/accumulation/split/prepass flag | Keep FA route trace diagnostic. Future FA work needs shader/resource instrumentation, per-KV tail timing, or structural long-KV work that remains on coopmat1 and changes main-loop cost; do not repeat mask-opt disable, analytic-causal prepass elimination, f16acc, SHMEM staging, forced split-k, nearby `Bc` probes, larger-Br cm1, or `D_split` flips |
| Q4 KV cache (`q4_0/q4_0`) | E076 Vulkan 32k KV gate: q4 safe-force baseline `9.8493`; q8 `9.1102`; f16 `8.8361` | Q4 KV reduces memory footprint and bandwidth enough to fit long context and keep attention viable; higher precision KV increases memory pressure | Current preferred Q3 KV route. Treat q4 as part of that active lane, not as an optional side note |
...
| Vulkan Q3_K prompt route | E061 12k prompt-heavy: Vulkan `4.2206 TPS` vs ROCm `6.3327 TPS`; decode-biased Vulkan was faster (`35.2850` vs ROCm `27.9781`). E100/E102 32k valid spec-none: Vulkan `10.5230`, ROCm `10.8879`. E128 64k best Vulkan is `1.3406 TPS` vs ROCm `1.5545`, with `MUL_MAT q3_K` at `47.79%`; active pipeline remains `matmul_q3_k_f32_f16acc_aligned_l`. E133 shape summary: `MUL_MAT q3_K` parsed total `42684.45 ms`; top shapes `m=17408,n=1024,k=5120` (`20338.69 ms`) and `m=5120,n=1024,k=17408` (`11289.87 ms`) are `74.1%` of parsed Q3_K time. E134 route ceiling: FFN gate/up alone needs `2.234x` local to close the lane, all Q3_K needs `1.357x`, and Q3_K+FA needs `1.172x`. E135 real-server trace proves the active graph exposes `63 x q3_K SWIGLU` FFN gate/up candidates with `m=17408,n=1024,k=5120`. E136 models dual-A/same-B FFN fusion: base dual-A LDS `29696 B`, accumulators `16 -> 32`, local ceiling with unchanged A proxy `1.417x`, projected `1.4466 TPS`. E137 rejects current dual-N/same-A: clean restored default pp7488 `974.92`, `113 VGPR`; `niter2` pp7488 `855.29`, `120 VGPR`, no scratch. E139 rejects existing per-node predequant: direct pp7488 `969.61`; all-large `Q3_K -> fp16 prealloc_x -> f16 matmul` `743.65`; only `m>=17000` `832.27`; only `k>=17000` `929.40`; f16 route resources `77 VGPR`, `44 SGPR`, `22528 B LDS`, `0 scratch`. E140 rejects existing matmul split-K for `m=5120,n=1024,k=17408`: direct `968.74`, split-K2 `966.21`, split-K4 `964.46`. E143 rejects large-N warptiles despite lower static A-dequant proxy: default `974.19`; `bn192-wn96` `760.78`, `139 VGPR / 25088 B LDS`; `bn256-*` about `660`, `165 VGPR / 29696 B LDS`; `bn192-wm128-wn96` spills scratch and collapses to `137.71`. E144 rejects BK16 despite lower resources: default `972.77`, `bk16` `587.52`, `70 VGPR / 12288 B LDS`; BK64 is static-rejected at `36864 B LDS`. E146 rejects BM256: default `972.84`, `bm256` `916.62`, `94 VGPR / 31744 B LDS`, no scratch. E147 rejects broad persistent layout: FFN fp16/int8 alternates need `+25.03 GiB`/`+9.09 GiB`; signed-nibble is only `+1.12 GiB` but low-confidence after SPIR-V and E088/E090 | Vulkan decode is competitive, but prompt-heavy Q3_K matmul remains one of the two 64k blockers alongside FA. Prior Q3_K shader/tile families around this route are heavily screened and many are negative. The evidence now says to target a whole Q3_K route branch rather than global tile churn, launch-only fusion, FFN fusion that only saves B/intermediate traffic, dual-N accumulation that buys A reuse with VGPR pressure, larger-N or larger-M tiles that trade traffic/workgroup reduction for VGPR/LDS/occupancy pressure, BK shrink that trades resources for too much K-loop/barrier overhead, existing predequant fallback that buys lower VGPR but pays temp/sync/global traffic, existing split-K where the hot shape already has enough workgroups, or broad persistent Q3_K fp16/int8 layout that does not fit the lane | Keep Vulkan as active 64k target. New Q3_K work must pass route-ceiling, graph-pattern, resource, shape-level perf, and A-side work-reduction gates before server A/B. Next Q3_K branch should wait for a topology that avoids fp16 temp/sync/reduce, larger-tile live-state blowups, near-limit LDS, broad extra VRAM, and extra barrier/reduce cadence; dual-A/same-B FFN fusion is a stack component only if resource proof is clean |
...
- The first easy FA gates are negative: disabling mask-opt regresses, forced
  ...
  pp. E145 rejects simple `D_split` retunes: `4/16` keep the same resource
  report but lose runtime. Future FA speed work should start from shader/body
  evidence inside the existing q4/q4 coopmat1 route, not another trivial env
  toggle, an f16 KV cache route, larger `Br` without a live-state redesign, or
  `D_split` flipping.
