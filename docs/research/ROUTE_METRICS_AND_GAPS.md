...
| FlashAttention on Vulkan 64k | E128 perf trace: `FLASH_ATTN_EXT 33965.16 ms`, `38.03%` of traced Vulkan time. E131 route trace: `flash_attn_f32_f16_aligned_f32accq4_0`, `coopmat1`, q4/q4, `Br=16,Bc=64,D_split=8,row_split=4`, main `N=1024`, `KV=1024..57344`, `split_k=1`, `use_mask_opt=1`. Mask-opt disable regressed to `639.44` prompt tok/s; forced f16acc full run was `1.3380 TPS` vs E128 best `1.3406` | At 64k FA becomes a first-class bottleneck, but the easy toggles are already negative. Mask optimization pays for itself, split-k is not active on main chunks, and f16acc does not move full wall TPS. The remaining cost is likely shader memory/resource behavior over long KV, not a trivial tile/accumulation flag | Keep FA route trace diagnostic. Future FA work needs shader/resource instrumentation or structural long-KV work; do not repeat mask-opt disable, f16acc, or nearby `Bc` probes |
...
| Vulkan Q3_K prompt route | E061 12k prompt-heavy: Vulkan `4.2206 TPS` vs ROCm `6.3327 TPS`; decode-biased Vulkan was faster (`35.2850` vs ROCm `27.9781`). E100/E102 32k valid spec-none: Vulkan `10.5230`, ROCm `10.8879`. E128 64k best Vulkan is `1.3406 TPS` vs ROCm `1.5545`, with `MUL_MAT q3_K` at `47.79%`; active pipeline remains `matmul_q3_k_f32_f16acc_aligned_l` | Vulkan decode is competitive, but prompt-heavy Q3_K matmul remains one of the two 64k blockers alongside FA. Prior Q3_K shader/tile families around this route are heavily screened and many are negative | Keep Vulkan as active 64k target. New Q3_K probes must pass prebuild/static gates and avoid already rejected warptiles/arithmetic-only rewrites |
...
- The first easy FA gates are negative: disabling mask-opt regresses and
  forced f16acc does not beat the full-run best. Future FA speed work should
  start from shader/resource evidence, not another trivial env toggle.
