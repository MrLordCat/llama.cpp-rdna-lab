...
- Реализовано: LLAMA_VM_MTP_KV_LAST_F16=N (f16 для последних N KV-слоёв).
...
- Decode spec=none (N==1, scalar FA, короткий промпт): f8 30.4 vs f16 29.5
...
- Декод всегда скалярный (vk_pipeline.inc: «scalar is faster than coopmat
  when N==1»); f8-декант — FMUL/FADD без аппаратных dot (D1).

### Гипотезы и направления (приоритет сверху вниз)
1. **D1: f8-dot для скалярного декода** — конверсия f8→f16 на лету в
   регистрах + аппаратный f16-dot (V_DOT2_F32_F16) вместо FMA; K/V-чтения
   остаются f8 (вдвое меньше bandwidth). Ожидание: декод f8 >= f16.
   Риск: register pressure; надо проверить V_DOT2 на RDNA4 (gfx1201).
2. **D2: shmem-стаггинг f8 на декоде** — shmem_staging=1 форсится для f8
   (vk_pipeline.inc:965); для N==1 декод K-стаггинг в shmem f16: проверить
   выключение (прямые f8-чтения + dequantize4, как в prefill-ветке
   flash_attn.comp:324-349) — возможно, дешевле staging.
3. **D3: coopmat1 на декоде для f8** — пересмотреть запрет (n_rows==1 →
   scalar): с 16-строчными коопматами 1 строка = 16x пустая работа, но
   эмуляция fp8-коопматов может быть быстрее скалярного деканта при
   memory-bound 49K+; A/B на 49K и 98K.
4. **D4: гибрид N=4 vs 8 для MTP-декода** — после D1-D3 перемерить кривую
   N; цель: N минимальное при acc >= 75%; при быстром f8-декоде N=4 даст
   f8-скорость + дравт-качество.
5. **D5: V-only гибрид** (K f8 + V f16 последних N слоёв) — вдвое меньше
   f16-чтений, чем полный гибрид; проверить acceptance (шум V vs K).
6. **D6: дравт-префилл f8** — MTP KV (слой nextn) сейчас всегда f16:
   перевести на f8 (окно 256) → быстрее длинные prefill с MTP; проверить
   acceptance дравта.
7. **D7: prefill-мелочи** — битовый энкодер уже ~12 instr/компонент;
   проверить KV-запись одним проходом (copy_to_quant) и wg-размеры P5 на
   49K/98K; target: +3-5% сверх текущего паритета.
