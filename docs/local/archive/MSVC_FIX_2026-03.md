# Историческая заметка: определение MSVC (2026-03)

> [!WARNING]
> Это описание зафиксировало состояние старого GUI-пути. Методы проверки
> MSVC всё ещё существуют в `gui/dependency_installer.py`, но автоматический
> вызов `initialize_msvc_env()` из GUI больше не используется. Документ не
> является текущим build-контрактом; актуальные команды находятся в
> [`../BUILD_CHEATSHEET.md`](../BUILD_CHEATSHEET.md) и `AGENTS.md`.
