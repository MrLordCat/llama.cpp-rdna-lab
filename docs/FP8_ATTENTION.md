...
> **Устарело (2026-09-14, E348):** автоматическое выставление N удалено —
> хвост теперь строго opt-in. На ROCm драфт получает скрытое состояние NextN
> через device handoff и KV таргета не читает, поэтому хвост стоил
> `+3552 MiB` и `-13.5%` префилла при `ctx 151552` ни за что, а смена точности
> посередине кэша ломала агентные сессии с тулами на `f8_e4m3`. Описанные ниже
> измерения кривой N остаются валидными для Vulkan-лейнов, где выигрыш
> acceptance был измерен; включать их следует явным `LLAMA_VK_MTP_KV_LAST_F16`. См.
