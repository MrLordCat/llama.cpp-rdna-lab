...
- Sweep notes:
  ...
  - E053 closed the post-C01 no-code selection gate. Trace-off control was `prefill-e053-control-r1 = 11.7681 TPS`. Fresh split timing repeated the Q3_K profile: `src0 32.66%`, `src1 6.85%`, `GEMM 60.49%`, with the same Q3_K `6144x5120@ncols2048` shape at `1841.00 ms` and `src0 1440.13 ms` (`78.23%`). Kernel-full large-prompt trace: `MUL_MAT 64.50%`, Q3_K `MUL_MAT 54.21%`, `GATED_DELTA_NET 14.76%`, `FLASH_ATTN_EXT 4.81%`, H06 QKV/RoPE-adjacent slice `18.48%`.
  - E053 decision: next branch is Q3_K dequant/layout only if the design can plausibly deliver about `>=25%` local dequant/layout win or a combined dequant+GEMM effect. A `20%` local Q3_K dequant-only improvement projects only `1.0172x` aggregate (`11.9700 TPS`); `25%` projects `1.0207x` (`12.0112 TPS`).
  - E054 added default-off `GGML_TRACE_CUBLAS_SPLIT_DETAIL=1` to separate temporary fp16 allocation from conversion. Diagnostic result: Q3_K `src0_alloc_ms = 6.12 ms` (`0.18%` of Q3_K `src0`), `src0_convert_ms = 3370.32 ms` (`99.80%`). The Q3_K `6144x5120@ncols2048` target had `src0_convert_ms = 1430.88 ms` and effectively zero allocation cost. Decision: allocation/reuse is not the next bottleneck; continue with guarded Q3_K fp16 conversion/layout work in `convert.cu`.
  - E055 tested and reverted a 64-thread-preserving Q3_K half2 store prototype. Trace-off r1 was `11.86 TPS`, but split-detail showed only `3317.06 ms` Q3_K `src0_convert_ms` vs E054 `3370.32 ms` (`-1.58%`) and only `1425.06 ms` vs `1430.88 ms` (`-0.41%`) on the target `6144x5120@ncols2048` shape. Decision: simple half2 store packing is below the local keep gate; do not repeat this variant.
  - E056 changed the small-gain policy: a same-session r3 `~0.5-1.5%` aggregate win can be useful as a stackable opt-in knob, even if it cannot clear the old `>=2%` standalone/default gate. Rechecked H26 with r3: default `11.6726 TPS`, half2 `11.6375 TPS` (`-0.30%`), so H26 stays rejected. E057 then screened explicit scalar unroll4 and rejected it at r1 (`11.58 TPS`), below the same-session control.
  - E058 rechecked old runtime `noise-level` rejects under the small-gain policy. Fresh r3 control was `11.6132 TPS`. `ROCBLAS_USE_HIPBLASLT=1` reached `11.6618 TPS` (`+0.42%` aggregate) but median task TPS was below control, so it is watchlist-only, not an opt-in recommendation. `GGML_CUDA_NO_PINNED=1` regressed to `11.5714 TPS`; hipBLASLt+no-pinned regressed to `11.5915 TPS`.
