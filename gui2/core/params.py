...
SCHEMA: tuple[Param, ...] = (
    ...
    Param("cache_type_k", "Key cache type", "choice", G_CACHE, "--cache-type-k",
          choices=KV_TYPES, skip_default=True),
    Param("cache_type_v", "Value cache type", "choice", G_CACHE, "--cache-type-v",
          choices=KV_TYPES, skip_default=True),
    ...
    Param("metrics", "Prometheus /metrics", "bool", G_SERVER, "--metrics", emit="presence",
          help="a /metrics endpoint that any scraper can read"),
    ...
    Param("mmproj_offload", "Offload mmproj to GPU", "bool", G_MODEL, "--no-mmproj-offload",
          emit="absence",
          help="the vision weights stay in VRAM rather than falling back to RAM"),
    ...
    Param("embeddings", "Embedding mode", "bool", G_SERVER, "--embeddings", emit="presence",
          help="the server answers embedding requests instead of chat"),
    Param("flash_attn", "Flash attention", "choice", G_ADVANCED, "--flash-attn",
          choices=("auto", "on", "off"), skip_default=True),
    Param("no_warmup", "Skip warm-up pass", "bool", G_ADVANCED, "--no-warmup", emit="presence",
          help="no run-up prompts before the first answer"),
    Param("no_mmap", "Disable mmap", "bool", G_ADVANCED, "--no-mmap", emit="presence",
          help="copy the model out of the file instead of mapping it"),
    Param("disable_thinking", "Disable thinking", "bool", G_ADVANCED, "--chat-template-kwargs",
          emit="composite",
          help="asks the template to keep reasoning out of the answer"),
