...
def guide(plan: WorkerPlan, this_machine: str = "") -> Guide:
    ...
    here = f" ({this_machine})" if this_machine else ""
    return Guide(
        steps=(
            "Build llama.cpp on the other machine with -DGGML_RPC=ON, using the "
            "backend its own GPU needs.",
            f"Run the command below there. It binds to every network card, so this machine"
            f"{here} can reach it.",
            "Allow the port through the other machine's firewall — a worker that is "
            "running but blocked looks exactly like one that is not running.",
            "Put its address in the RPC workers box here as host:port, then press "
            "Check to confirm the version and the memory it is offering.",
        ),
        warnings=(
