...
def worker_sh(plan: WorkerPlan) -> str:
    """The one file to run on a Linux or macOS worker: opens the port, starts the worker.

    Same contract as the .bat: everything the other machine needs in one
    file, next to the rpc-server binary. The firewall rule is best-effort
    (ufw/firewalld, only when run as root) because there is no single
    firewall on desktop Linux; the worker binds its port either way, and the
    rule only matters for the machines that reach it.
    """
    argv = ['"$(dirname "$0")/rpc-server"']
    argv += ["-H", "0.0.0.0" if plan.open_to_network else "127.0.0.1", "-p", str(plan.port)]
    if plan.devices:
        argv += ["-d", ",".join(plan.devices)]
    if plan.threads:
    ...
    lines = [
        "#!/usr/bin/env bash",
        f"# Generated by GUI 2.0 - rpc-server worker on port {plan.port}",
        "set -u",
        'if [ ! -x "$(dirname "$0")/rpc-server" ]; then',
        '  echo "rpc-server is missing - put this file next to it."; exit 1; fi',
        f"PORT={plan.port}",
        'if [ "$(id -u)" = "0" ]; then',
        '  command -v ufw >/dev/null 2>&1 && ufw allow "$PORT/tcp" >/dev/null 2>&1',
        '  command -v firewall-cmd >/dev/null 2>&1 && firewall-cmd --add-port="$PORT/tcp" >/dev/null 2>&1',
        "fi",
        "ADDR=\"\"",
        "if command -v ip >/dev/null 2>&1; then ADDR=$(ip -4 -o addr show scope global 2>/dev/null | awk '{print $4}' | cut -d/ -f1 | head -n1); fi",
        "if [ -z \"$ADDR\" ] && command -v hostname >/dev/null 2>&1; then ADDR=$(hostname -I 2>/dev/null | awk '{print $1}'); fi",
        'if [ -z "$ADDR" ]; then echo "Could not find this machine\'s IPv4 address - run ip addr and type it into the GUI manually."; fi',
        "echo",
        'echo "Your address for the GUI is:"',
        'echo "  $ADDR:$PORT"',
        'if [ -n "$ADDR" ]; then',
        '  command -v wl-copy >/dev/null 2>&1 && printf "%s:%s" "$ADDR" "$PORT" | wl-copy',
        '  command -v xclip >/dev/null 2>&1 && printf "%s:%s" "$ADDR" "$PORT" | xclip -selection clipboard',
        '  echo "If wl-copy or xclip is installed, the address is already on your clipboard: go to the GUI, open More than one machine and paste it into the address box."',
        "fi",
        "echo",
        'echo "This window must stay open while the GUI uses this machine - press Ctrl+C to stop the worker."',
        "echo",
        *(["echo Cache is ON: the first model load copies the weights here, later loads read them from this machine's disk - much faster."]
          if plan.cache else []),
        " ".join(argv),
    ]
    return "\n".join(lines) + "\n"
...
def guide(plan: WorkerPlan, this_machine: str = "") -> Guide:
    ...
    return Guide(
        steps=(
            f"Run the generated rpc-worker-{plan.port}.bat (Windows) or "
            f"rpc-worker-{plan.port}.sh (Linux/macOS) on the other machine — on "
            "Windows as Administrator. It opens the firewall where it can, starts "
            "the worker, prints the machine's own address and puts it on the "
            "clipboard where a clipboard tool is installed.",
