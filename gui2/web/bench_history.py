...
SORT_KEYS = {
    ...
    "mtp_acc": lambda row: row[0].mtp_acc_pct,
        ...
        for representative_row, run_build, placement, scenarios in chosen:
            ...
            run_rows += [
                  Tr(Td(A("▸", hx_get=f"/autotune/history/run?series_id={quote(series_id)}",
                   ...
                   Td(draft),
                   Td(f"{representative_row.mtp_acc_pct:.1f}%" if representative_row.mtp_accepted else "—",
                        title=representative_row.mtp_accepted or ("no speculation" if representative_row.spec_mode == "none" else "no acceptance data"),
                        cls="num"),
...
def run_detail(config: AppConfig, series_id: str) -> Tr:
    ...
    for row in scenarios:
        ...
        scenario_rows.append(Tr(
            ...
            Td(f"{row.mtp_acc_pct:.1f}%" if row.mtp_accepted else "—",
               title=row.mtp_accepted or "", cls="num"),
            ...
            Div(Table(
                Thead(Tr(*[Th(name, cls="num" if name not in {"When", "Run", "Status"}
                           ...
                           for name in ("When", "Run", "Lane", "Prefill t/s", "Decode t/s",
                                        "Total t/s", "MTP acc", "Context", "Status", "Spec")])),
                Tbody(*scenario_rows),
            ), cls="table-wrap"),
