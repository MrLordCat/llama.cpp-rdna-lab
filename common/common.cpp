...
common_init_result::common_init_result(common_params & params) :
    ...
    if (params.fit_params) {
        ...
        if (fit_status != COMMON_PARAMS_FIT_STATUS_SUCCESS) {
            LOG_ERR("%s: failed to fit model/context parameters to device memory; use -fit off to force startup with the requested parameters\n", __func__);
            return;
        }
    }
...
struct llama_model_params common_model_params_to_llama(common_params & params) {
    ...
    mparams.load_mtp                    = std::find(params.speculative.types.begin(), params.speculative.types.end(),
