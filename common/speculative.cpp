...
struct common_speculative_state {
    ...
    size_t n_ver_drafts = 0; // number of verification rounds observed by this implementation.
    size_t n_ver_tokens = 0; // number of draft tokens verified by the target model.
...
struct common_speculative_state_dflash : public common_speculative_state {
    ...
    common_speculative_state_dflash(enum common_speculative_type type,
        ...
        cross_window      = std::max(1, params.dflash_cross_ctx);
        ...
        if (!contract_ok) {
            ...
            LOG_INF("dflash: state ready (block_size=%d mask_token=%d n_target_layers=%d n_target_features=%d cross_window=%d)\n",
                    block_size, (int) mask_token_id, n_target_layers, n_target_features, cross_window);
        }
    }
        ...
        switch (config.type) {
            ...
            case COMMON_SPECULATIVE_TYPE_DFLASH: {
                impls.push_back(std::make_unique<common_speculative_state_dflash>(
                    config.type, ctx_tgt, ctx_dft, config.params));
                break;
            }
...
void common_speculative_reject(common_speculative * spec, uint16_t n_attempted, uint16_t n_accepted) {
...
void common_speculative_print_stats(const common_speculative * spec) {
    ...
    for (const auto & impl : spec->impls) {
        ...
        if (impl->n_ver_tokens > 0) {
            std::ostringstream oss;
            oss << ", #verify drafts = " << impl->n_ver_drafts
                << ", #verify tokens = " << impl->n_ver_tokens
                << ", eff acceptance = " << std::fixed << std::setprecision(2)
                << (100.0 * (double) impl->n_acc_tokens / (double) impl->n_ver_tokens) << "%";
            str_verify = oss.str();
        }
