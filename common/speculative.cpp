...
#define SPC_TRC(fmt, ...) LOG_DBG("spec %12.*s: " fmt, 12, __func__, __VA_ARGS__)
...
const std::map<std::string, common_speculative_type> common_speculative_type_from_name_map = {
    ...
    {"eagle3",        COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE3},
    {"draft-eagle3",  COMMON_SPECULATIVE_TYPE_DRAFT_EAGLE3},
    {"mtp",           COMMON_SPECULATIVE_TYPE_DRAFT_MTP},
    {"draft-mtp",     COMMON_SPECULATIVE_TYPE_DRAFT_MTP},
    {"dflash",        COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH},
...
static bool common_speculative_model_arch_is(const llama_model * model, const char * arch) {
    char buf[64];
...
struct common_speculative_impl {
    ...
    int64_t t_process_us = 0; // total time spent processing target outputs in this implementation in microseconds.
    ...
    virtual void set_process_enabled(bool /*enabled*/) {}
...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    int  trace_chunk_count = 0;
    bool trace_first_draft = true;
    bool trace_dflash      = false;
        ...
        , params(params.draft)
    {
        ...
        if (mask_token_id < 0) {
            char buf[32] = {};
            if (llama_model_meta_val_str(model_dft, "dflash-draft.dflash.mask_token_id", buf, sizeof(buf)) >= 0) {
        ...
        LOG_INF("%s: - dflash_ubatch=%d (ctx_ubatch=%d, override with LLAMA_DFLASH_UBATCH)\n",
                __func__, n_dflash_ubatch, (int) llama_n_ubatch(ctx_dft));
    ...
    bool process(const llama_batch & batch_in) override {
        ...
        const int32_t n_ubatch = n_dflash_ubatch;
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            for (int32_t offset = 0; offset < n_rows; offset += n_ubatch) {
                ...
                const bool trace_chunk = trace_dflash && trace_chunk_count < 180;
                if (trace_chunk) {
                    ...
                    for (const float value : features_buf) {
                    ...
                    const size_t good = features_buf.size() - bad;
                    if (good == 0) {
                        min_value = 0.0f;
                        max_value = 0.0f;
                    }

                    SPC_INF("DFlash chunk %d features: elems=%zu, bad=%zu, min=%g, max=%g, mean=%g\n",
                            trace_chunk_count, features_buf.size(), bad, (double) min_value, (double) max_value,
                            good > 0 ? sum / (double) good : 0.0);
                }
                ...
                const int32_t pos0 = batch_in.pos ? batch_in.pos[i_batch_beg[seq_id] + offset] : -1;
                if (trace_chunk) {
                    SPC_INF("DFlash chunk %d begin: seq_id=%d, pos0=%d, n_chunk=%d, offset=%d, n_embd_enc=%d, n_embd_dec=%d\n",
                            trace_chunk_count, (int) seq_id, (int) pos0, (int) n_chunk, (int) offset, (int) n_embd_enc, (int) n_embd_dec);
                }
                ...
                if (trace_chunk) {
                    SPC_INF("DFlash chunk %d encode complete\n", trace_chunk_count);
                }
                ...
                if (trace_chunk) {
                    SPC_INF("DFlash chunk %d encoder ready: inp_g=%p, injecting %d rows\n",
                            trace_chunk_count, (const void *) inp_g, (int) n_chunk);
                }
                ...
                const llama_pos trim_pos = batch_inject.pos[0];
                if (!llama_memory_seq_rm(mem_dft, seq_id, trim_pos, -1)) {
                    LOG_ERR("%s: failed to trim DFlash draft KV before injection (seq=%d, pos=%d)\n",
                            __func__, (int) seq_id, (int) trim_pos);
                    return false;
                }
                ...
                if (trace_chunk) {
                    SPC_INF("DFlash chunk %d injection complete\n", trace_chunk_count);
                }
                ++trace_chunk_count;
            }
        }
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        if (trace_dflash && trace_first_draft) {
            SPC_INF("DFlash first draft decode: n_tokens=%d, n_max=%d, block_size=%d\n",
                    batch.n_tokens, params.n_max, block_size);
        }
        ...
        if (ret != 0) {
            ...
            for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
                ...
                if (!llama_memory_seq_rm(mem_dft, seq_id, dparams[seq_id].n_past, -1)) {
                    LOG_WRN("%s: failed to trim DFlash draft KV after decode failure (seq=%d, pos=%d)\n",
                            __func__, (int) seq_id, (int) dparams[seq_id].n_past);
                }
                last_draft_pos[seq_id] = -1;
            }
            return;
        }
        llama_synchronize(ctx_dft);
        ...
        if (trace_dflash && trace_first_draft) {
            SPC_INF("%s", "DFlash first draft decode complete\n");
        }
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            for (int32_t i = 1; i < n_block_tokens; ++i) {
                if (trace_dflash && trace_first_draft && i == 1) {
                    SPC_INF("DFlash first draft sampling: seq_id=%d, beg=%d, n_block_tokens=%d\n",
                            (int) seq_id, (int) beg, (int) n_block_tokens);
                }
        ...
        if (trace_dflash && trace_first_draft) {
            SPC_INF("%s", "DFlash first draft sampling complete\n");
        }
        if (trace_first_draft) {
            trace_first_draft = false;
        }
    }

    void accept(llama_seq_id seq_id, uint16_t n_accepted, bool is_other) override {
        if (is_other || seq_id < 0 || seq_id >= (llama_seq_id) last_draft_pos.size()) {
            return;
        }

        const llama_pos draft_pos = last_draft_pos[seq_id];
        if (draft_pos < 0) {
            return;
        }

        auto * mem_dft = llama_get_memory(params.ctx_dft);
        const llama_pos keep_end = draft_pos + n_accepted + 1;
        if (!llama_memory_seq_rm(mem_dft, seq_id, keep_end, -1)) {
            LOG_WRN("%s: failed to trim rejected DFlash KV (seq=%d, pos=%d, n_accepted=%u)\n",
                    __func__, (int) seq_id, (int) keep_end, (unsigned) n_accepted);
        }
        last_draft_pos[seq_id] = -1;
    }
...
struct common_speculative_impl_draft_mtp : public common_speculative_impl {
    ...
    std::vector<llama_pos>          pending_pos; // [n_seq] position of pending_h, -1 if invalid
        ...
        , params(params.draft)
    {
        ...
        const bool has_ctx_other = llama_get_ctx_other(ctx_dft) == ctx_tgt;
...
std::vector<common_speculative_type> common_speculative_types_from_names(const std::vector<std::string> & names) {
    ...
    for (const auto & name : names) {
        if (name == "ngram-mtp" || name == "ngram_mtp") {
...
common_params common_base_params_to_speculative(const common_params & params) {
    ...
    const bool has_dflash = std::find(params.speculative.types.begin(),
                                      params.speculative.types.end(),
                                      COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH) != params.speculative.types.end();
    result.n_outputs_max = has_dflash ? result.n_batch : params.n_parallel;
...
common_speculative_init_result::common_speculative_init_result(
    ...
    cparams.n_rs_seq  = spec_dflash ? dflash_rs_seq : 0;
    cparams.ctx_other = ctx_tgt;

    std::string model_path;
    if (has_draft) {
        model_path = params.speculative.draft.mparams.path;
        LOG_DBG("%s: loading draft model '%s'\n", __func__, model_path.c_str());
        ...
        LOG_DBG("%s: creating MTP draft context against the target model '%s'\n", __func__, model_path.c_str());
...
void common_speculative_set_process_enabled(common_speculative * spec, bool enabled) {
    if (spec == nullptr) {
        return;
    }

    for (auto & impl : spec->impls) {
        impl->set_process_enabled(enabled);
    }
}
