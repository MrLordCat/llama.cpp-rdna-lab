...
static llama_model * llama_model_mapping(llm_arch arch, const llama_model_params & params) {
    switch (arch) {
        ...
        default:
            throw std::runtime_error(std::string("model architecture is not enabled in this build: '") +
                    llm_arch_name(arch) + "'");
    }
}
...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    // upstream keeps the input layer on the CPU because the embedding lookup is
    // cheap; this fork puts it on the first GPU device by default so a fully
    // VRAM-resident model (e.g. qwen35 248320-vocab token_embd, ~682 MiB) does
    // not hold host RAM. LLAMA_NO_INPUT_OFFLOAD=1 restores the upstream behavior.
    const bool no_input_offload = getenv("LLAMA_NO_INPUT_OFFLOAD") != nullptr;
    if (!no_input_offload && !devices.empty()) {
        ggml_backend_dev_t dev = devices.front().dev;
        pimpl->dev_input = { dev, &pimpl->gpu_buft_list.at(dev) };
        LLAMA_LOG_INFO("%s: offloading input layer to device %s (LLAMA_NO_INPUT_OFFLOAD=1 to disable)\n",
