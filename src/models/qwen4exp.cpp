...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    // n_layer_all: every layer incl. nextn; used as table length by per-layer KV
    ...
    // (the GGUF may also carry attention.recurrent_layers, but this engine keeps
    //  the layer pattern in hparams.recurrent_layer_arr; see qwen3next)
    {
...
ggml_tensor * llama_model_qwen4exp::graph::build_attn_qsa(
    ...
    ggml_tensor * cur = build_attn_mha(q, k, v, nullptr, kq_mask_top_k, nullptr, nullptr, nullptr, kq_scale, il);
    cb(cur, "kqv_out", il);
...
ggml_tensor * llama_model_qwen4exp::graph::build_layer_attn_linear(
    ...
    // write the new delta-net state back to the recurrent cache
