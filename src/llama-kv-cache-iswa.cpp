...
llama_kv_cache_iswa::llama_kv_cache_iswa(
    ...
    const  layer_reuse_cb & reuse,
                     bool   is_mtp_context) : hparams(model.hparams), unified(unified) {
    ...
    kv_base = std::make_unique<llama_kv_cache>(
            ...
            0, LLAMA_SWA_TYPE_NONE, filter_base, reuse, is_mtp_context);
    ...
    kv_swa = std::make_unique<llama_kv_cache>(
            ...
            hparams.n_swa, hparams.swa_type, filter_swa, reuse, is_mtp_context);
}
...
ggml_backend_dev_t llama_kv_cache_iswa::get_layer_device(int32_t il) const {
    if (auto * dev = kv_base->get_layer_device(il)) {
        return dev;
    }
    return kv_swa->get_layer_device(il);
}
