...
FAMILY_SUMMARIES = [
    ...
    (
        "130k major topology",
        "D002-D033",
        "Vulkan reached the old 2 TPS target through the D012 q3quad + GLU stack and is now retargeted to `2.4 TPS`; ROCm is recentered at `1.5200 TPS` and paused after D013-D027.",
        "ROCm ub256, raw-storage escape, forced cublas/dequant, no-mmap, src1 quant reuse, y32/w2, GLU-only, dense staging, b4 loads, Q3Flash active-shape promotion, wider-N scalar Q3Flash, dual-Y MMQ, vbuffer single-chunk, multi-row WMMA Q3Flash, upstream-stock rollback, streaming dequant+rocBLAS chunking, pair-only FFN SwiGLU WMMA, naive whole-FFN streaming, expanded persistent Q3_K layout, compact signed-nibble unpack-only layout, Vulkan activation-only/naive-streaming whole-FFN, old all-Q3 storage/helper/Q8/tile families, compact Q3S layout-body work, FA-only pivot, and q3-octa/LOAD_VEC_A=8 repeat all failed.",
        "Continue Vulkan only with a true Q3_K compute body or compressed-dot route: D032 says FA can be a stack component only after Q3 has roughly `1.18-1.20x` local point/static evidence, and D033 rejects wider per-invocation Q3 dequant; reopen ROCm only with a stronger topology proof.",
    ),
    (
        "ROCm decode parity",
        "E149-E201",
...
def main() -> int:
    ...
    lines: list[str] = [
        ...
        "| --- | --- |",
        "| Active dense 130k target | `Qwen3.6-27B-Q3_K_S`, `ctx=131072,b=512,q4_0/q4_0,spec=none`, cold/no-reuse/no-prime, `real-context-chars=24576`, `max_tokens=16`, thinking on. Vulkan D012 `ub=256` is the active baseline at `2.0013 TPS`; the new target is `2.4 TPS`. ROCm `ub=128` baseline is `1.5200 TPS` and paused after D013-D027. |",
