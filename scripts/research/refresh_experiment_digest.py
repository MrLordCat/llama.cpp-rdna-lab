...
def main() -> int:
    ...
    lines: list[str] = [
        ...
        "| --- | --- |",
        "| Active dense 130k target | `Qwen3.6-27B-Q3_K_S`, `ctx=131072,b=512,q4_0/q4_0,spec=none`, cold/no-reuse/no-prime, `real-context-chars=24576`, `max_tokens=16`, thinking on. Vulkan D012 `ub=256` remains the active speed baseline at `2.0013 TPS`; D036 is the guarded q4/q4 default recovery at `1.9410 TPS` with decode `40.2033 tok/s`, still not a new speed baseline. D037 keeps q8/q8 only as explicit stability/offline opt-in via `LLAMA_VK_KV_HOST_AUTO_Q8=1` and rejects mixed q4/q8 on current Vulkan/RDNA. D038/D039 keep tool-call reliability compensation default-on, disableable with `LLAMA_SERVER_TOOL_CALL_THINKING_GUARD=0`; D039 BFCL-lite measured `24/25` default vs `16/25` explicit thinking, but it is not a speed route. The new speed target is `2.4 TPS`. ROCm `ub=128` baseline is `1.5200 TPS` and paused after D013-D027. |",
