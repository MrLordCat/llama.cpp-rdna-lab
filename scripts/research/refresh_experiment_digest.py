...
FAMILY_SUMMARIES = [
    ...
    (
        ...
        "E045-E059, E103-E106, E228-E256, E292",
        "The practical bottleneck is large Q3_K prefill through staged fp16 + rocBLAS/hipBLAS. E292 keeps packed padded-Q3_K dequant as the HIP default (`-8.14%` conversion, `+0.72-1.52%` prompt), while GEMM still dominates.",
        ...
        "Further material Q3_K gains need a new compressed-GEMM/layout topology with point proof before server integration.",
    ),
    (
        "ROCm RDNA4 FlashAttention",
        "E293",
        "Full Qwen D256 prompt dispatch now uses rocWMMA with 16 columns per block; bundled-header discovery makes the route reproducible after clean installs.",
        "Generic tile was a fresh-build configuration regression, not a better RDNA4 default.",
        "Keep rocWMMA on; use `GGML_HIP_ROCWMMA_FATTN=OFF` only as a rollback or unsupported-platform gate.",
    ),
    (
        "130k major topology",
        "D002-D039",
        "Vulkan reached the old 2 TPS target through the D012 q3quad + GLU stack; D036 hardens those route pieces plus direct pinned host-KV as the q4/q4 default recovery (`1.9410 TPS`, decode `40.2033 tok/s`), still below D012. D037 keeps q8/q8 only as explicit stability/offline opt-in. D038 promotes q4/q3 tool-call reliability compensation to default-on, with rollback via `LLAMA_SERVER_TOOL_CALL_THINKING_GUARD=0`; D039 transfers it to BFCL-lite public data (`24/25` default vs `16/25` explicit thinking) and identifies repeated parallel-call undercoverage as the next quality failure class. E293 reopens ROCm long-prompt work: its matched `ctx=131072` 53.5k lane reaches `1557.94 prompt tok/s` after restoring rocWMMA.",
