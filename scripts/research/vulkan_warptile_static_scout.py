...
MEASURED = {
    ...
    "bk16": "static-only: doubles K-loop/barrier count; likely low priority unless pipeline resources improve materially",
    "bk64": "static-only: halves K-loop/barrier count but doubles Q3 shared-memory footprint; requires resource/pipeline proof before build",
    ...
    "bn256": "static-only: halves A dequant proxy for N=1024; high-ceiling if LDS/VGPR stay acceptable",
    "bn256-wn128": "static-only: halves A dequant proxy while keeping prepared block at 256; likely higher accumulator pressure",
    "bn256-wm128": "static-only: halves A dequant proxy while keeping prepared block at 256; likely higher accumulator pressure",
